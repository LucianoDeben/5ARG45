...
for min_n in min_n_values:
    logging.info(f"Processing TF activity inference with min_n = {min_n} and algorithm = {fixed_algorithm}")

    # Apply TF activity inference for each dataset using the current min_n and the Dorothea network.
    datasets = {
        name: (
            run_tf_activity_inference(data, dorothea_net, min_n=min_n, algorithm=fixed_algorithm),
            target,
        )

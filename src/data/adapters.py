...
class LINCSAdapter(DataAdapter):
    ...
    def read_data(self) -> Tuple[np.ndarray, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            self.compound_info = self.compound_info[["pert_id", "cmap_name", "canonical_smiles"]]
            self.compound_info.set_index("pert_id", inplace=True)
            self.compound_info = self.compound_info[~self.compound_info.index.duplicated(keep='first')]
            logger.info(f"Compound info shape after dropping duplicates: {self.compound_info.shape}")
            
            # Load column metadata
            logger.info("Loading column metadata...")
            self.col_metadata = pd.read_csv(self.gene_info_file, sep="\t")
            
            return self.expression_data, self.row_metadata, self.compound_info, self.col_metadata
    ...
    def write_data(self, output_path: str) -> None:
        ...
        with h5py.File(output_path, "w") as f_out:
            ...
            meta_col_grp.create_dataset("id", data=np.array(self.col_symbols, dtype="S"))
...
class LINCSDatasetFactory:
    ...
    def create_dataset(config: Dict[str, str]) -> LINCSAdapter:
        ...
        """
        required_keys = [
            "expression_file", "row_metadata_file", 
            "compound_info_file", "gene_info_file"
        ]
        ...
        metadata = DatasetMetadata(
            ...
            description=config.get("description", "LINCS gene expression data in .gctx format"),

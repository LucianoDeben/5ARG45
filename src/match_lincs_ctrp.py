...
class LincsCtprMatcher:
    ...
    def load_data(self):
        ...
        self.siginfo = pd.read_csv(self.config["siginfo"], sep="\t")
    ...
    def prepare_final_data(self):
        ...
        self.siginfo_concat["smiles"] = self.per_compound_smiles_concat
        self.siginfo_concat["viability"] = self.viability_concat
        ...
        # Create combined row metadata
        self.row_metadata = pd.concat([
            self.siginfo_slice_1.set_index("sig_id", drop=False),
            self.siginfo_slice_2.set_index("sig_id", drop=False)
        ])
        ...
        self.row_metadata['viability_clipped'] = self.row_metadata.viability.clip(0, 1)
...
def write_matched_data_to_gctx(
    expression_data, row_metadata, row_ids, col_ids, gene_symbols, output_path, metadata
):
    """
    ...
    with h5py.File(output_path, "w") as f_out:
        ...
        col_id_array = np.array(col_ids, dtype="S")
        gene_symbol_array = np.array(gene_symbols, dtype="S")
        
        meta_col_grp.create_dataset("id", data=col_id_array)
        meta_col_grp.create_dataset("gene_symbol", data=gene_symbol_array)

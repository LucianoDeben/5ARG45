...
data_dict_74709 = {'acts': acts_concat
...
for i, row in pair_curves.iterrows():    
    ...
    pair_per_cpd_post_qc = lookup_pair_per_cpd_post_qc(pair_experiment_id, pair_master_cpd_id)
    
    p = row[['p1_center', 'p2_slope', 'p4_baseline']]

    vis_data_and_curve(pair_per_cpd_post_qc.cpd_conc_umol
                    , pair_per_cpd_post_qc.cpd_avg_pv
                    , p
                    , i)

dose_log2 = np.log2(pair_siginfo.pert_dose)

plt.scatter(dose_log2

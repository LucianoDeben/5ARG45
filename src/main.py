...
def main():
    ...
    # Setup logging using experiment config
    setup_logging(log_level=config["experiment"].get("log_level", "INFO"))

    # Apply command-line overrides where provided
    config["training"]["num_runs"] = (
        args.runs if args.runs is not None else config["training"].get("num_runs", 5)
    )
    config["training"]["epochs"] = (
        args.epochs if args.epochs is not None else config["training"]["epochs"]
    )
    config["data"]["nrows"] = (
        args.nrows if args.nrows is not None else config["data"].get("nrows")
    )
    ...
    config["data"]["random_seed"] = (
        args.seed if args.seed is not None else config["data"]["random_seed"]
    )
    config["paths"]["results_dir"] = (
        args.output_dir if args.output_dir else config["paths"]["results_dir"]
    )
    config["training"]["learning_rate"] = (
        args.lr if args.lr is not None else config["training"]["learning_rate"]
    )
    ...
    try:
        ...
        logging.error(f"Error configuring training components: {e}")
        return
    ...
    multi_trainer = MultiRunTrainer(
        ...
        scheduler_class=scheduler_factory,  # Pass factory with parameters
        criterion=criterion,

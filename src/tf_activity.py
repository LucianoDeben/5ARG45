...
for min_n in min_n_values:
    # For each min_n, loop over each algorithm
    for algorithm in algorithms:
        ...
        dataloaders = {
            name: {
                split: create_dataloader(X, y, batch_size=1028, shuffle=True)
        ...
        for name, loaders in dataloaders.items():
            ...
            # Create the model
            logging.info(f"Training FlexibleFCNN on {name} feature set with min_n = {min_n} and algorithm = {algorithm}...")
            ...
            # Use a composite key that includes the feature set, min_n, and algorithm
            results[f"{name}_min_n_{min_n}_algo_{algorithm}"] = fcnn_metrics

        # Store the results for the current min_n and algorithm in the overall results dictionary
        all_results[f"min_n_{min_n}_algo_{algorithm}"] = results

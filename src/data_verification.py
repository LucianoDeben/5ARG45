...
with h5py.File(h5_file, "r") as f:
    ...
    assert len(row_ids) == data_shape[0], f"Number of row IDs ({len(row_ids)}) does not match number of rows in data ({data_shape[0]})."
    ...
    assert len(col_ids) == data_shape[1], f"Number of column IDs ({len(col_ids)}) does not match number of columns in data ({data_shape[1]})."
    ...
    row_meta_field = meta_row_grp["cell_mfc_name"][:]  # Adjust the field name as needed.
    logging.info(f"First 5 values in 'cell_mfc_name': {row_meta_field[:5]}")
    
    # 6. Verify that the data is stored as float32.
    assert data_dtype == np.float32, f"Data dtype expected to be float32, got {data_dtype}"
